      ...
      <div className="flex flex-wrap justify-center gap-3">
        ...
        <div className="bg-slate-800 rounded-lg p-3 w-80">
          ...
          <details className="mb-2">
            ...
            <div className="mt-2 space-y-2 p-2 bg-slate-700 rounded">
              <div>
                <label className="text-xs text-slate-300">G: {G.toFixed(4)} p.u.</label>
                <input 
                  type="range" 
                  min="0.01" 
                  max="0.18" 
                  ...
                  onChange={(e) => {
                    ...
                    const scale = newG / (G || 0.01);
                    setSystemConfig({
                      ...systemConfig,
                      P_load1: systemConfig.P_load1 * scale,
                      P_load2: systemConfig.P_load2 * scale,
                      P_load3: systemConfig.P_load3 * scale,
                    });
                  }}
              ...
              <div>
                <label className="text-xs text-slate-300">Bc: {Bc.toFixed(4)} p.u.</label>
                <input 
                  type="range" 
                  min="0.06" 
                  max="0.28" 
                  ...
                  value={Bc}
                  onChange={(e) => {
                    const newBc = parseFloat(e.target.value);
                    const scale = newBc / (Bc || 0.01);
                    setSystemConfig({
                      ...systemConfig,
                      Qc_cap1: systemConfig.Qc_cap1 * scale,
                      Qc_cap2: systemConfig.Qc_cap2 * scale,
                      Qc_cable: systemConfig.Qc_cable * scale,
                    });
                  }}
                  className="w-full h-1 bg-slate-600 rounded appearance-none"
                />
        ...
        <div className="bg-slate-800 rounded-lg p-2">
          ...
          <svg width={svgW} height={svgH}>
            ...
            {/* 操作方向の矢印 */}
            <defs>
              <marker id="arrowO" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto">
            ...
            {/* 負荷遮断の方向 */}
            <line x1={toX(G)} y1={toY(Bc) + 25} x2={toX(G) - 40} y2={toY(Bc) + 25} 
              stroke="#f97316" strokeWidth={2} markerEnd="url(#arrowO)" />
            <text x={toX(G) - 20} y={toY(Bc) + 40} fill="#f97316" fontSize="8" textAnchor="middle">負荷遮断</text>
            
            {/* コンデンサ切離の方向 */}
            <line x1={toX(G) + 25} y1={toY(Bc)} x2={toX(G) + 25} y2={toY(Bc) + 40} 
              stroke="#3b82f6" strokeWidth={2} markerEnd="url(#arrowB)" />
            <text x={toX(G) + 45} y={toY(Bc) + 20} fill="#3b82f6" fontSize="8" textAnchor="middle">コンデンサ</text>
            <text x={toX(G) + 45} y={toY(Bc) + 30} fill="#3b82f6" fontSize="8" textAnchor="middle">切離</text>
...
export default InteractiveStabilityAnalysis;
